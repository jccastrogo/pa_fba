...
def knockut_gene_essentiality_test(model: cobra.Model):
    ...
    for gene_id in gene_ids:
        ...
        knockout_model.objective = knockout_model.reactions.get_by_id('bio1_biomass')
...
def map_flux_to_genes_operator_conscious(model: cobra.Model, solution: cobra.Solution):
    ...
    for reaction in model.reactions:
        ...
        if reaction.gene_reaction_rule not in ['', 'nan']:
            gpr = reaction.gene_reaction_rule
            if ' or ' in gpr:
                # if there is an 'or' relationship, count the number of 'or' operators plus 1 and divide the flux by that number to avoid overestimating the flux
                num_or_in_gpr = gpr.count(' or ') + 1
                flux = flux / num_or_in_gpr
            # removes parentheses and logical operators from GPR string to allow to easy splitting
            for chr in ['(', ')', ' and', ' or']:
                gpr = gpr.replace(chr, '')
            # adds the absolute value of the flux to the gene in the gene_flux_dict; adds the gene first it if not present
...
def main():
    ...
    protein_expression_data_df = pd.read_csv('/data2/R_Choudhury/pa_fba/individual_scripts/P380_All_Detected_Proteins_SAF.tsv', sep='\t')
